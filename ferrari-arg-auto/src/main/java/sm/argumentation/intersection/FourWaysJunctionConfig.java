...
public final class FourWaysJunctionConfig implements Debatable {
     ...
     * @param cars
     */
    public FourWaysJunctionConfig(final Junction junction, final List<CrossingCar> cars) {
    ...
    public FourWaysJunctionConfig(final String junctionName, final CrossingPolicy policy, final RSU<?>... rsu) {
        final Map<WAY, Road> roads = new HashMap<>();
        ...
        for (final WAY way : WAY.values()) {
            roads.put(way, new Road(new BaseRoad(way.name(), lanes), rsus));
        }
        this.junction = new Junction(junctionName, roads, policy);
    ...
    public FourWaysJunctionConfig addCar(final UrgentCar car, final String roadName) {
        ...
        for (final WAY way : this.junction.getRoads().keySet()) {
            if (this.junction.getRoads().get(way) != null
                    && this.junction.getRoads().get(way).getRoad().getId().equals(roadName)) {
     ...
     */
    @Override
    public List<Proposition> argue(final AspicArgumentationTheory<PlFormula> t) {
        Proposition a = null;
        Proposition b = null;
        Proposition c = null;
        Proposition d = null;
